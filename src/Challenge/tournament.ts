...
router.post("/create/challenge", async (req: any, res: any) => {
  ...
  const verify = challenge.safeParse({
    name,
    memberqty,
    Dailystep,
    Amount,
...
router.post("/step/verification", async (req: any, res: any) => {
  ...
  while (i < challeng.days) {
    ...
    if (
      Stepmap[date.toISOString().split("T")[0]] < challeng.Dailystep ||
  ...
  try {
    ...
    if (trans == true) {
      ...
      if (challenge.type == "private") {
        ...
        const updatedRequest = challenge.Request.filter(
          (usert: string) => usert !== user.username
        );
        await prisma.challenge.update({
          where: {
            id: users.id,
    ...
    if (trans) {
      ...
      console.log(users);
      if (!users) {
        const transaction = await revertback(
          privatekey,
          decoded.signatures[0].publicKey.toBase58(),
          challenge.Amount
        );
        return res
          .status(200)
...
router.post("/challenge/acceptchallenge", async (req: any, res: any) => {
  ...
  if (!challenge?.Request[username]) {
    res.json({
      message:
        "You were not added to the Challenge Kindly ask the user to add you to challenge",
    });
    return;
  }
